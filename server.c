...
void * thread_main(void * arg_in)
{
    ...
    char buf[LINE_SIZE];
    int guess, result;
    char send_buf[MSG_BUF_SIZE + 10];

    // Send the maximum value to the client
    int max_value = gmn_get_max();
    snprintf(buf, sizeof(buf), "%d\n", max_value);
    if (send_all(sockfd, buf, strlen(buf)) == -1) {
        perror("Error sending max value to client");
        goto cleanup;
    }
    ...
    while (1) {
        ...
        if (recv_lines(sockfd, buf, sizeof(buf)) == -1) {
            perror("Error receiving guess from client");
            goto cleanup;
        }

        // Parse the guess
        if (sscanf(buf, "%d", &guess) != 1) {
            fprintf(stderr, "Invalid guess received\n");
            goto cleanup;
        }
        ...
        if (result == 0) {
            // Correct guess, send 0 and the final message
            snprintf(send_buf, sizeof(send_buf), "0%s", gmn_get_message(&gmn));
            if (send_all(sockfd, send_buf, strlen(send_buf)) == -1) {
                perror("Error sending final message to client");
                goto cleanup;
            }
            break; // Exit the loop
        } else {
            // Send the result code (-1 or 1)
