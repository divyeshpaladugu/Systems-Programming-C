...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p)
{
    ...
    // While any column is empty, wait on consume_cond
    while (p->counts[0] <= 0 || p->counts[1] <= 0 || p->counts[2] <= 0) {
        pthread_cond_wait(&p->consume_cond, &p->mutex);
    }

    // Remove items from the first row
    *a = p->buf[0][0];
    *b = p->buf[0][1];
    *c = p->buf[0][2];

    // Slide down the items in each column
...
{   
    ...
    while(!done)
    {
        ...
        // If the linked-list is empty, exit the loop
        pthread_mutex_unlock(&p->mutex);
        if (n == NULL) {
            done = 1;
            break;
...
int main(int argc, char *argv[])
{
    ...
    int total = 0;
    // Sum up the total items produced by all producer threads
