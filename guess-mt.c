...
// Provide a declaration for snprintf
...
void * thread_c(void * arg_in)
{
    ...
    arg->max = gmn_get_max();
    arg->status = S_MAX;
    pthread_cond_signal(&arg->cond_result); // signal that max is ready
...
void * thread_p(void *arg_in)
{
    ...
    do { 
        ...
        // wait for the result from thread_c
        pthread_mutex_lock(&arg->mutex);
...
int main(int argc, char *argv[])
{
    ...
    rv = pthread_create(&tid_c, NULL, thread_c, &arg);
    check_pthread_return(rv, "pthread_create for thread_c");

    rv = pthread_create(&tid_p, NULL, thread_p, &arg);
    check_pthread_return(rv, "pthread_create for thread_p");

    rv = pthread_join(tid_c, NULL);
    check_pthread_return(rv, "pthread_join for thread_c");

    rv = pthread_join(tid_p, NULL);
    check_pthread_return(rv, "pthread_join for thread_p");
